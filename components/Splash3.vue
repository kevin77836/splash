...
const sphereParams = reactive({
  ...
  maxLength: 12.0,      // 最大目標長度
  segmentDetail: 16,   // 幾何體細分
  minScale: 0,        // 最小縮放值(原點狀態)
  irregularity: 0,   // 不規則程度 (0-1)
  maxElongation: 1.1,  // 最大延展比例
  minElongation: 0.9   // 最小延展比例
});
...
function createSphere(direction, size) {
  ...
  const geometry = new THREE.SphereGeometry(
    size, 
    sphereParams.segmentDetail, 
    sphereParams.segmentDetail
  );
  
  // 計算不規則變形係數
  const xElongation = THREE.MathUtils.lerp(
    sphereParams.minElongation, 
    sphereParams.maxElongation, 
    Math.random()
  );
  
  const yElongation = THREE.MathUtils.lerp(
    sphereParams.minElongation, 
    sphereParams.maxElongation, 
    Math.random()
  );
  
  const zElongation = THREE.MathUtils.lerp(
    sphereParams.minElongation, 
    sphereParams.maxElongation, 
    Math.random()
  );
  ...
  const positions = basePositions.array;
  
  // 添加不規則變形
  for (let i = 0; i < positions.length; i += 3) {
    // 應用基本橢球體變形
    positions[i] *= xElongation;       // x軸
    positions[i + 1] *= yElongation;   // y軸
    positions[i + 2] *= zElongation;   // z軸
    
    // 添加不規則噪聲變形 (如果irregularity > 0)
    if (sphereParams.irregularity > 0) {
      const noise = (Math.random() - 0.5) * sphereParams.irregularity;
      positions[i] *= (1 + noise);
...
function updateSpheres() {
  ...
  for (let i = 0; i < spheres.length; i++) {
    ...
    // 計算時間差
    const deltaTime = currentTime - sphere.lastUpdateTime;
    sphere.lastUpdateTime = currentTime;
    ...
    if (currentPosition < 0.01) {
      ...
      sphere.mesh.scale.set(
        scale * (sphere.xElongation || 1), 
        scale * (sphere.yElongation || 1), 
        scale * (sphere.zElongation || 1)
      );
    }
    
    // 更新球體旋轉
    if (sphere.rotationSpeed && currentPosition > 0.1) {
      // 只有當球體脫離原點一定距離時才有旋轉
      const rotationFactor = Math.min(1, sphere.progress * 2); // 旋轉強度與進度相關
      
      sphere.mesh.rotation.x += sphere.rotationSpeed.x * deltaTime * rotationFactor;
      sphere.mesh.rotation.y += sphere.rotationSpeed.y * deltaTime * rotationFactor;
      sphere.mesh.rotation.z += sphere.rotationSpeed.z * deltaTime * rotationFactor;
    }
  }
}
