...
/**
 * 載入著色器檔案
 * @param {string} filePath - 著色器檔案路徑
 * @returns {Promise<string>} - 著色器代碼
 */
function loadShaderFile(filePath) {
 ...
 * @param {string} options.vertexShaderPath - 頂點著色器路徑
 * @param {string} options.fragmentShaderPath - 片元著色器路徑
 * @returns {Promise<THREE.ShaderMaterial>} - 著色器材質
 */
async function createShaderMaterial({ vertexShaderPath, fragmentShaderPath }) {
  try {
    console.log(`載入著色器: ${vertexShaderPath} 和 ${fragmentShaderPath}`);
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        resolution: { value: new THREE.Vector2(
          canvasContainer.value ? canvasContainer.value.clientWidth : window.innerWidth,
          canvasContainer.value ? canvasContainer.value.clientHeight : window.innerHeight
        )}
      },
      vertexShader,
      fragmentShader,
...
function updateShaderUniforms() {
  if (!material || !material.uniforms || !clock) return;
  ...
  if (material.uniforms.resolution) {
    const width = canvasContainer.value ? canvasContainer.value.clientWidth : window.innerWidth;
    const height = canvasContainer.value ? canvasContainer.value.clientHeight : window.innerHeight;
    
    if (material.uniforms.resolution.value.set) {
...
/**
 * 應用自定義著色器材質
 * @param {string} shaderType - 著色器類型: '2d' 或 '3d'
 */
async function applyCustomShader(shaderType = '3d') {
  if (!effect) return;
  
  try {
    let vertexShaderPath, fragmentShaderPath;
    
    if (shaderType === '2d') {
      // 2D版本使用簡單的頂點著色器和2D的片元著色器
      vertexShaderPath = '/shaders/basic3D.vert'; // 我們可以重用這個
      fragmentShaderPath = '/shaders/digitalRainbow.frag';
    } else {
      // 3D版本使用完整的頂點著色器和3D的片元著色器
      vertexShaderPath = '/shaders/basic3D.vert';
      fragmentShaderPath = '/shaders/digitalRainbow3D.frag';
    }
    
    // 創建著色器材質
    const customMaterial = await createShaderMaterial({
      vertexShaderPath,
      fragmentShaderPath
    });
    
    console.log(`應用${shaderType}著色器材質`);
    ...
    effect.material = customMaterial;
...
function initializeScene() {
  ...
  // 應用自定義著色器（預設使用3D版本）
  applyCustomShader('3d').then(success => {
    if (success) {
      console.log('3D著色器應用成功');
    } else {
      console.warn('3D著色器應用失敗，使用默認材質');
    }
  });
