...
const animParams = reactive({
  ...
  flowWaveAmplitudeFactor: 0,  // 波浪振幅因子
  flowWaveAmplitudeFactorAtStart: 0, // 原點狀態下的波浪振幅因子
...
function loadFontAndCreateText() {
  ...
  ttfLoader.load('/fonts/Orbitron-Regular.ttf', (json) => {
    ...
    // 創建第一個文字 - 位於左側
    textMesh1 = createText('Splash', new THREE.Vector3(-4, 0, -5), 1);
    scene.add(textMesh1);
    
    // 創建第二個文字 - 位於右側
    textMesh2 = createText('DigiLab', new THREE.Vector3(4, 0, -5), 1);
    scene.add(textMesh2);
  });
}
 ...
 * 創建文字物件
 */
function createText(text, position, size = 0.5) {
  ...
  mesh.position.set(position.x - textWidth / 2, position.y, position.z);
  return mesh;
}
