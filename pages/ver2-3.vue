...
const animParams = reactive({
  ...
  // 球體設定
  maxCenterBallStrength: 0,     // 中心球體最大強度
...
// --- 球體控制狀態 ---
let centerBallStrength = 0;  // 中心球體當前強度
let ballTargetStrength = 0;  // 中心球體目標強度
let ballShrinkStartTime = null; // 球體縮小開始時間
let ballGrowStartTime = null;  // 球體放大開始時間
...
function updateGlobalFlowState(currentTime) {
  ...
  if (globalFlowState === 'growing' && growStartTime !== null) {
    ...
    if (averageProgress >= 0.99) {
      ...
      // 球體強度設置為最小值
      centerBallStrength = animParams.minCenterBallStrength;
    ...
    if (currentTime >= shrinkEndTime) {
      ...
      // 球體強度設置為最大值
      centerBallStrength = animParams.maxCenterBallStrength;
...
function startSyncShrinking() {
  ...
  // 設置球體放大開始時間
  ballGrowStartTime = currentTime;
  // 設置球體目標強度為最大值
  ballTargetStrength = animParams.maxCenterBallStrength;
}
...
function startSyncGrowing() {
  ...
  // 設置球體縮小開始時間
  ballShrinkStartTime = currentTime;
  // 設置球體目標強度為最小值
  ballTargetStrength = animParams.minCenterBallStrength;
...
function calculateCenterBallStrength(currentTime) {
  if (!clock) return animParams.maxCenterBallStrength;
  ...
  // 計算球體強度
  if (globalFlowState === 'shrinking' && ballGrowStartTime !== null && shrinkStartTime !== null && shrinkEndTime !== null) {
    // 球體放大 - 與線條收縮同步
    const shrinkDuration = shrinkEndTime - shrinkStartTime;
    const timeSinceShrinking = currentTime - shrinkStartTime;
    const progress = Math.min(timeSinceShrinking / shrinkDuration, 1.0);
    
    // 從最小值逐漸增加到最大值
    centerBallStrength = animParams.minCenterBallStrength + progress * (animParams.maxCenterBallStrength - animParams.minCenterBallStrength);
    return centerBallStrength;
  } 
  else if (globalFlowState === 'growing' && ballShrinkStartTime !== null) {
    // 球體縮小 - 與線條生長同步
    const totalLines = ttParams.numLines + thParams.numLines;
    let totalGrowthProgress = 0;
    let activeLines = 0;
    ...
    // 球體強度與生長進度成反比
    centerBallStrength = animParams.maxCenterBallStrength * (1.0 - averageProgress);
    return centerBallStrength;
  } 
...
function handleButtonClick() {
  // 防止動畫進行中重複點擊
  if (globalFlowState === 'growing' || globalFlowState === 'shrinking') {
    console.log("動畫進行中，請等待完成");
    return;
  }
  ...
  if (isPlaying.value) {
    ...
    console.log("返回按鈕被點擊，開始收合動畫");
    startShrinkingAnimation();
  } else {
    // 播放按鈕被點擊 - 開始生長動畫
    isPlaying.value = true;
    console.log("播放按鈕被點擊，開始生長動畫");
...
function initializeCollapsedState() {
  ...
  console.log("初始化完成：線條收合狀態，中心球體最大");
}
