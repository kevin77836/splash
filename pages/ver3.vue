<template>
  ...
  const utils = {
      ...
      ball: {
          calculateCenterStrength(currentTime) {
              if (!clock) return 0.5; // 確保初始時也返回一個有效值
              ...
              // 如果播放還未開始，顯示固定大小的球體
              if (!isPlaying.value) {
                  return 0.5; // 使用固定值而不是基於 flowParams 的值，確保可見性
              }
  ...
  onMounted(() => {
    ...
    Promise.all([
      ...
      // 即使有錯誤，仍嘗試啟用播放按鈕
      isReady.value = true;
      console.log("嘗試啟用播放按鈕，但部分資源可能未完全載入");
    });
